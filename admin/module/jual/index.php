 		...
 		<div class="card card-primary">
 			<div class="card-header bg-primary text-white">
 				<h5><i class="fa fa-shopping-cart"></i> KASIR
 					...
 					<button type="button" class="btn btn-success float-right mr-2" data-toggle="modal" data-target="#tambahBarangModal">
 				...
 				<div id="keranjang" class="table-responsive">
 								...
 								<tr>
 									...
 									<td>
 										...
 										<a href="fungsi/hapus/hapus.php?jual=jual&id=<?php echo $isi['id_penjualan']; ?>&brg=<?php echo $isi['id_barang']; ?>
											&jml=<?php echo $isi['jumlah']; ?>" class="btn btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus item ini dari keranjang?')"><i class="fa fa-times"></i>
 										</a>
 									</td>
 								</tr>
 							<?php $no++;
 					...
 					<div id="kasirnya">
 						<table class="table table-stripped">
								...
								if (!empty($_GET['nota'] == 'yes')) {
									...
									if (!empty($bayar)) {
										...
										if ($bayar >= $total) {
											...
											// Hapus semua item dari tabel penjualan
											$sql_delete = "DELETE FROM penjualan";
											$row_delete = $config->prepare($sql_delete);
											$row_delete->execute();

											$_SESSION['success_message'] = "Pembayaran Berhasil! Keranjang telah dikosongkan. Kembalian: Rp " . number_format($hitung, 0, ',', '.');
											$_SESSION['kembalian'] = $hitung; // Store the change amount in the session
											echo "<script>window.location.href='index.php?page=jual';</script>";
											exit;
										} else {
											$_SESSION['error_message'] = "Uang Kurang ! Rp. " . number_format(abs($hitung), 0, ',', '.');
											echo "<script>window.location.href='index.php?page=jual';</script>";
											exit;
										}
									}
 							...
 							<tr>
 								<td>Kembali</td>
 								<td><input type="text" class="form-control" value="<?php echo isset($_SESSION['kembalian']) ? 'Rp ' . number_format($_SESSION['kembalian'], 0, ',', '.') : (isset($hitung) ? 'Rp ' . number_format($hitung, 0, ',', '.') : 'Rp 0'); ?>" readonly></td>
 								<td></td>
 								<td>
 									<a href="print.php?nm_member=<?php echo $_SESSION['admin']['nm_member']; ?>
&bayar=<?php echo isset($bayar) ? $bayar : '0'; ?>&kembali=<?php echo isset($_SESSION['kembalian']) ? $_SESSION['kembalian'] : '0'; ?>&total=<?php echo $total_bayar; ?>" target="_blank">
 	...
 	<!-- Di bagian bawah file, sebelum tag penutup </body> -->
 	...
 	<script>
 		function updateMaxStock() {
 			...
 			var maxStock = selectedOption.getAttribute('data-stok');

 			jumlahInput.max = maxStock;
 			jumlahInput.placeholder = "Max: " + maxStock;
 		}
 		...
 		function validateStock() {
 			...
 			var jumlahInput = document.getElementById('jumlah');
 			var selectedOption = select.options[select.selectedIndex];
 			var maxStock = parseInt(selectedOption.getAttribute('data-stok'));
 			var jumlah = parseInt(jumlahInput.value);

 			if (jumlah > maxStock) {
 				alert("Jumlah melebihi stok yang tersedia. Stok tersedia: " + maxStock);
 				return false;
 			}
 			return true;
 		}

 		updateMaxStock(); // Call this function initially to set the placeholder for the first item
 	</script>
 		...
 		<div class="modal-dialog" role="document">
 			<div class="modal-content">
 				...
 				<div class="modal-body">
 					<form method="POST" action="fungsi/tambah/tambah.php?penjualan=tambah" onsubmit="return validateStock()">
 						<div class="form-group">
 							<label for="nama_barang">Nama Barang</label>
 							<select class="form-control" id="nama_barang" name="nama_barang" required onchange="updateMaxStock()">
 								...
 								<?php
									...
									while ($barang = $row_barang->fetch(PDO::FETCH_ASSOC)) {
										echo "<option value='" . $barang['id_barang'] . "' data-stok='" . $barang['stok'] . "'>" . $barang['nama_barang'] . " (Stok: " . $barang['stok'] . ")</option>";
									}
									?>
 							</select>
 						</div>
